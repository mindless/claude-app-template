...
<execution_context>
When prompts are executed via /run-prompt, each agent runs in its own isolated git worktree. This enables:
- Safe parallel execution without merge conflicts
- Clean atomic merges back to the main branch
- Automatic cleanup after completion

Generated prompts should be aware they'll run in isolation and may need to account for this.
</execution_context>
...
- **Frontend involvement**: Does this task involve UI/frontend work or require visual validation?
...
- UI components, styling, layout, animations, visual anything → frontend task

<frontend_task_indicators>
A task is a frontend task if ANY of these apply:
- Creates or modifies React/Vue/Svelte/etc. components
- Involves CSS, Tailwind, styling, or visual design
- Requires layout, responsive design, or animations
- Needs visual validation or screenshot verification
- Mentions UI, UX, frontend, client-side, browser
- Involves Framer Motion, GSAP, or animation libraries
</frontend_task_indicators>

<dependency_analysis_for_multiple_prompts>
When creating multiple prompts, analyze dependencies to determine execution strategy:

**Can run in PARALLEL if:**
- Different files/directories with no overlap
- No shared state or database dependencies
- Independent features that don't import each other
- Example: "Add user avatar component" + "Add notification badge" (different UI areas)

**Must run SEQUENTIALLY if:**
- One task creates files/types another task imports
- Shared database tables or API endpoints
- One task modifies core utilities used by another
- Order matters for correctness
- Example: "Create auth context" → "Add protected routes" (second needs first)

**Mark prompts with execution hints:**
- Add `<!-- execution: parallel-safe -->` for parallel-capable prompts
- Add `<!-- execution: depends-on: [prompt-numbers] -->` for sequential dependencies
</dependency_analysis_for_multiple_prompts>
</adaptive_analysis>
...
**Frontend Task Requirements (ALWAYS include for frontend tasks):**

If the task involves frontend/UI work or requires visual validation, ALWAYS include this block in the generated prompt:

```xml
<frontend_tooling>
This is a frontend task. Use the following approach:

1. **Visual Reconnaissance First**: Before making changes, use the Playwright MCP to capture the current visual state:
   - Take screenshots of affected pages/components
   - Document the current layout, spacing, colors, and behavior

2. **Use Frontend Design Skill**: Invoke `/frontend-design` skill for:
   - Design system compliance
   - Responsive design patterns
   - Animation and interaction guidance
   - Accessibility considerations

3. **Implement Changes**: Make the required changes following the design skill's guidance

4. **Visual Validation**: After implementation, use Playwright MCP to:
   - Capture new screenshots
   - Compare before/after visually
   - Verify responsive behavior at multiple breakpoints
   - Check animations and interactions work as expected

5. **Critique and Iterate**: Review the visual output critically:
   - Does it match the design intent?
   - Are there visual regressions?
   - Is the implementation polished and production-ready?
</frontend_tooling>
```

**Execution Hints (ALWAYS include for multiple prompts):**

Add execution metadata at the top of each prompt:
```xml
<!-- execution-metadata
parallel-safe: true|false
depends-on: [list of prompt numbers if sequential]
touches-files: [list of files/directories this prompt modifies]
-->
```
